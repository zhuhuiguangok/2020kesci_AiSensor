...
for seed in seeds:
  ...
  for fold, (xx, yy) in enumerate(kfold.split(x, y)):
      ...
      early_stopping = EarlyStopping(monitor='val_acc',
                                    ...
                                    mode='max',
                                    patience=12)
      if not os.path.exists('./models/conv2d_withlabelsmoothing'):
        os.makedirs('./models/conv2d_withlabelsmoothing')  
      checkpoint = ModelCheckpoint(f'./models/conv2d_withlabelsmoothing/fold{fold}.h5',
                                  monitor='val_acc',
                                  verbose=0,
                                  mode='max',
                                  save_best_only=True)
      ...
      model.load_weights(f'./models/conv2d_withlabelsmoothing/fold{fold}.h5')
...
np.savetxt('stacking/conv2d_withlabelsmoothing.txt',proba_t)